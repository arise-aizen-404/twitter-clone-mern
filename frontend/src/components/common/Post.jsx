...
const Post = ({ post }) => {
  ...
  const { mutate: likePostMutation, isPending: isLiking } = useMutation({
    ...
    onSuccess: (updatedLikes) => {
      // if (isLiked) toast.success("Post unliked successfully");
      // else toast.success("Post liked successfully");
      // this is not best ui experience, bcoz it refetch all posts
      // queryClient.invalidateQueries({ queryKey: ["posts"] });
  ...
  const handlePostComment = (e) => {
    e.preventDefault();
    if (!isCommenting) commentPostMutation();
  };

  const handleLikePost = () => {
    if (!isLiking) likePostMutation();
  };
          ...
          <div className="flex justify-between mt-3">
            <div className="flex gap-4 items-center w-2/3 justify-between">
              ...
              >
                ...
                {!isLiking && !isLiked && (
                  <FaRegHeart className="w-4 h-4 cursor-pointer group-hover:text-pink-500 text-slate-500" />
                )}
                {!isLiking && isLiked && (
